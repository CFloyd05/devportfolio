...
A buffer op-amp is used after the DAC so it's output is not loaded by the following section.
...
This section compares the output volage from the DAC to a set-point voltage controlled by a potentiometer. This op-amp circuit provides both proportional and integral control.
...
The driver stage uses a BJT to control the current sent to the motor based off the output of the previous section. A flyback diode protects the circuit from the inductive nature of the motor.

Attached to the motor is a slotted disc, so that it's rotation can be measured with an LED and phototransistor. This produces a pulse train which is inputted to the counter in section 2). The disc I used had 10 slots, meaning there are 10 pulses per revolution.
...
After assembling the circuit, I proved it's functionality, such as the adjustable speed set-point and integral control, to the course staff.
